...
if ! docker buildx inspect nanopdf-builder >/dev/null 2>&1; then
    echo "Creating multi-arch builder: nanopdf-builder"
    docker buildx create --name nanopdf-builder --use --bootstrap
else
    docker buildx use nanopdf-builder
fi
...
case "$1" in
    deb)
        build_multiarch "$SCRIPT_DIR/Dockerfile.debian" "nanopdf-deb-builder" "$PROJECT_DIR/dist"
        ;;
    rpm)
        build_multiarch "$SCRIPT_DIR/Dockerfile.redhat" "nanopdf-rpm-builder" "$PROJECT_DIR/dist"
        ;;
    all)
        $0 deb && $0 rpm
        ...
        ;;
    clean)
        docker rmi $(docker images -q 'nanopdf-*-builder-*') 2>/dev/null || true
        docker buildx rm nanopdf-builder 2>/dev/null || true
